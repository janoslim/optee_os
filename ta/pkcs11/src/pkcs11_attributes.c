...
static const uint32_t rsa_public_key_optional[] = {
	PKCS11_CKA_MODULUS, PKCS11_CKA_PUBLIC_EXPONENT,
};

static const uint32_t rsa_private_key_optional[] = {
	PKCS11_CKA_MODULUS, PKCS11_CKA_PUBLIC_EXPONENT,
...
static const uint32_t ec_public_key_optional[] = {
	PKCS11_CKA_EC_POINT,
...
static const uint32_t ec_private_key_optional[] = {
	PKCS11_CKA_VALUE,
	PKCS11_CKA_EC_POINT,
	// temporarily until DER support
	PKCS11_CKA_EC_POINT_X, PKCS11_CKA_EC_POINT_Y,
						...
						struct obj_attrs *temp)
{
	...
	rc = set_optional_attributes(out, temp, public_key_optional,
				     ARRAY_SIZE(public_key_optional));
	if (rc)
		return rc;
	...
	case PKCS11_CKK_RSA:
		...
		optional = rsa_public_key_optional;
		mandated_count = ARRAY_SIZE(rsa_public_key_mandated);
		optional_count = ARRAY_SIZE(rsa_public_key_optional);
		break;
	case PKCS11_CKK_EC:
		mandated = ec_public_key_mandated;
		optional = ec_public_key_optional;
		mandated_count = ARRAY_SIZE(ec_public_key_mandated);
		optional_count = ARRAY_SIZE(ec_public_key_optional);
		break;
	default:
		EMSG("Invalid key type %#"PRIx32"/%s",
		     get_key_type(*out), id2str_key_type(get_key_type(*out)));
						 ...
						 struct obj_attrs *temp)
{
	...
	rc = set_optional_attributes(out, temp, private_key_optional,
				     ARRAY_SIZE(private_key_optional));
	if (rc)
		return rc;

	switch (get_key_type(*out)) {
	case PKCS11_CKK_RSA:
		optional = rsa_private_key_optional;
		optional_count = ARRAY_SIZE(rsa_private_key_optional);
		break;
	case PKCS11_CKK_EC:
		mandated = ec_private_key_mandated;
		optional = ec_private_key_optional;
		mandated_count = ARRAY_SIZE(ec_private_key_mandated);
		optional_count = ARRAY_SIZE(ec_private_key_optional);
		break;
	default:
		EMSG("Invalid key type %#"PRIx32"/%s",
		     get_key_type(*out), id2str_key_type(get_key_type(*out)));
